			   ...
			   size_t count, loff_t *ppos)
{
	...
	struct timespec ts;
	//printk("Path being read: %s\n",dentry->d_name.name);
	lower_file = wrapfs_lower_file(file);
//	printk("Last modified time: %ld\n",file_inode(lower_file)->i_mtime.tv_sec);
//	modifiedTimeInterval
	getnstimeofday(&ts);
//	printk("Current time: %ld",ts.tv_sec);
	if(ts.tv_sec-modifiedTimeInterval<file_inode(lower_file)->i_mtime.tv_sec){
		printk("Wait %ld seconds more\n",file_inode(lower_file)->i_mtime.tv_sec-ts.tv_sec+modifiedTimeInterval);
		return -EACCES;	
	}
	if(!allowExecution && inode_permission(file_inode(lower_file), MAY_EXEC)==0){
		printk("Allowexec is false and executes\n");
		return -EACCES;
	}
	//printk("Perm: %d\n",inode_permission(file_inode(lower_file), MAY_EXEC));
...
static int wrapfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	if(!allowAccessToLog){
        	dentry_path_raw(dentry, buffer, 100);
		for(i=0;i<100;i++){
			if(buffer[i]=='/' && buffer[i-1]==0){
				if(buffer[i+1]=='v'&&buffer[i+2]=='a'&&buffer[i+3]=='r'&&buffer[i+4]=='/'&&
					buffer[i+5]=='l'&&buffer[i+6]=='o'&&buffer[i+7]=='g'){
					kfree(buffer);
					return -EACCES;
				}
			}
        ...
        kfree(buffer);
	//printk("reading directory %s",dentry->d_name.name);
	lower_file = wrapfs_lower_file(file);
