			 ...
			 umode_t mode, bool want_excl)
{
	...
	/////////////////////////////////////////////////////////////////////////
	//printk("Permissions: %d\n",d_inode(lower_parent_dentry)->i_opflags);
//	printk("Last modified time: %ld\n",wrapfs_lower_inode(dir)->i_mtime.tv_sec);
	/*printk("Lower dentry path: %s\n",lower_dentry->d_name.name);
	printk("Before: %d\n",err);
        err = inode_permission(wrapfs_lower_inode(dir), MAY_EXEC);
        printk("After: %d\n",err);
	if(err){
		printk("Has permissions to execute. Returned: %d\n",err);
		err=-EACCES;
		goto out;
	}
*/
...
//	err=-EACCES;	goto out;
	...
	/*
	if(ghost_mode){
                printk("Creating in ghost mode");
                char* buffer;
                vfs_getxattr(lower_dentry, "GHOST", buffer, 4);
                printk("RESULT: %s\n",buffer);
        }*/
